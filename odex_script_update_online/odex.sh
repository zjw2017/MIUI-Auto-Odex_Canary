...
# MIUI ODEX项目贡献者：柚稚(zjw2017) & 冷洛(DavidPisces)
set_perm() {
...
export PATH=/data/adb/modules/miuiodex/system/bin:$PATH
if [ ! -f /storage/emulated/0/Android/MIUI_odex/RootImplement ]; then
  echo "! 必备文件丢失，请重刷MIUI ODEX 脚本更新模块"
  exit
else
  source /storage/emulated/0/Android/MIUI_odex/RootImplement
  if [ "$RootImplement" == "KernelSU" ] || [ "$RootImplement" == "APatch" ]; then
    MODPATH=/data/miuiodex
  else
    MODPATH=/data/adb/modules/miuiodex
  fi
fi
[ ! -f /data/adb/modules/odex_script_update_online_zjw2017/module.prop ] && echo "! 必备文件丢失，请重刷MIUI ODEX 脚本更新模块" && exit
[ ! -f /data/adb/modules/odex_script_update_online_zjw2017/customize_odex.sh ] && echo "! 必备文件丢失，请重刷MIUI ODEX 脚本更新模块" && exit
[ ! -f /data/adb/modules/odex_script_update_online_zjw2017/uninstall.sh ] && echo "! 必备文件丢失，请重刷MIUI ODEX 脚本更新模块" && exit
[ ! -f /data/adb/modules/odex_script_update_online_zjw2017/META-INF/com/google/android/update-binary ] && echo "! 必备文件丢失，请重刷MIUI ODEX 脚本更新模块" && exit
[ ! -f /data/adb/modules/odex_script_update_online_zjw2017/META-INF/com/google/android/updater-script ] && echo "! 必备文件丢失，请重刷MIUI ODEX 脚本更新模块" && exit
...
version=$(grep -w "version" /data/adb/modules/odex_script_update_online_zjw2017/module.prop | cut -d '=' -f2)
versionCode=$(grep -w "versionCode" /data/adb/modules/odex_script_update_online_zjw2017/module.prop | cut -d '=' -f2)
...
if [ -z "$HyperOS_version_code" ]; then
  OS_version="MIUI"
  modelversion="${MIUI_modelversion#V}"
else
  OS_version="Hyper"
  modelversion="$HyperOS_modelversion"
fi
mkdir -p $logfile
pm list packages -f -a >/storage/emulated/0/Android/MIUI_odex/packages.txt
...
dumpsys overlay | grep mBaseCodePath | awk '{print $NF}' >/storage/emulated/0/Android/MIUI_odex/overlay.txt
while IFS= read -r line; do
  line=$(printf '%s\n' "$line" | sed 's/[][\/.^$*]/\\&/g')
  sed -i "/${line}/d" /storage/emulated/0/Android/MIUI_odex/packages.txt
done <"/storage/emulated/0/Android/MIUI_odex/overlay.txt"
rm -rf /storage/emulated/0/Android/MIUI_odex/overlay.txt
...
if [[ "$choose_odex" != 3 ]]; then
  ...
  for a in /system/framework/*.jar; do
    ...
    jarhead=${a%.*}
    echo "- 开始处理$a"
    dex2oat --dex-file=/system/framework/"$jarhead".jar --compiler-filter="$odex_mode" --instruction-set=arm64 --oat-file="$MODPATH"/system/framework/oat/arm64/"$jarhead".odex
    dex2oat --dex-file=/system/framework/"$jarhead".jar --compiler-filter="$odex_mode" --instruction-set=arm --oat-file="$MODPATH"/system/framework/oat/arm/"$jarhead".odex
    echo "- 已完成对$a的处理"
  done
  if [ -d /product/framework ]; then
    ...
    for b in /product/framework/*.jar; do
      ...
      jarhead=${b%.*}
      echo "- 开始处理$b"
      dex2oat --dex-file=/product/framework/"$jarhead".jar --compiler-filter="$odex_mode" --instruction-set=arm64 --oat-file="$MODPATH"/system/product/framework/oat/arm64/"$jarhead".odex
      dex2oat --dex-file=/product/framework/"$jarhead".jar --compiler-filter="$odex_mode" --instruction-set=arm --oat-file="$MODPATH"/system/product/framework/oat/arm/"$jarhead".odex
      echo "- 已完成对$b的处理"
    done
  fi
  if [ -d /system_ext/framework ]; then
    ...
    for c in /system_ext/framework/*.jar; do
      ...
      jarhead=${c%.*}
      echo "- 开始处理$c"
      dex2oat --dex-file=/system_ext/framework/"$jarhead".jar --compiler-filter="$odex_mode" --instruction-set=arm64 --oat-file="$MODPATH"/system/system_ext/framework/oat/arm64/"$jarhead".odex
      dex2oat --dex-file=/system_ext/framework/"$jarhead".jar --compiler-filter="$odex_mode" --instruction-set=arm --oat-file="$MODPATH"/system/system_ext/framework/oat/arm/"$jarhead".odex
      echo "- 已完成对$c的处理"
    done
  fi
  if [ -d /vendor/framework ]; then
    ...
    for l in /vendor/framework/*.jar; do
      ...
      jarhead=${l%.*}
      echo "- 开始处理$l"
      dex2oat --dex-file=/vendor/framework/"$jarhead".jar --compiler-filter="$odex_mode" --instruction-set=arm64 --oat-file="$MODPATH"/system/vendor/framework/oat/arm64/"$jarhead".odex
      dex2oat --dex-file=/vendor/framework/"$jarhead".jar --compiler-filter="$odex_mode" --instruction-set=arm --oat-file="$MODPATH"/system/vendor/framework/oat/arm/"$jarhead".odex
      echo "- 已完成对$l的处理"
    done
  fi
      ...
      if [ -n "$record" ]; then
        ...
        if [ "$(unzip -l "$apk_path" | grep classes.dex)" != "" ]; then
          if [[ "$apk_source" == "data" ]]; then
            ...
            dex2oat --dex-file="$apk_path" --compiler-filter="$odex_mode" --instruction-set="$apk_abi" --oat-file="$apk_dir"/oat/"$apk_abi"/base.odex
            echo "- 已完成对$app_list的odex分离处理"
          else
            mkdir -p "$MODPATH""$apk_dir"/oat/"$apk_abi"
            dex2oat --dex-file="$apk_path" --compiler-filter="$odex_mode" --instruction-set="$apk_abi" --oat-file="$MODPATH""$apk_dir"/oat/"$apk_abi"/"$apk_name".odex
            echo "- 已完成对$app_list的odex分离处理"
          fi
        fi
        ((success_count++))
    ...
    for d in /system/app/*; do
      ...
      if [ -f /system/app/"$d"/"$d".apk ]; then
        if [ "$(unzip -l /system/app/"$d"/"$d".apk | grep classes.dex)" != "" ]; then
          ...
          echo "- 开始处理$d"
          dex2oat --dex-file=/system/app/"$d"/"$d".apk --compiler-filter="$odex_mode" --instruction-set="$apk_abi" --oat-file="$MODPATH"/system/app/"$d"/oat/"$apk_abi"/"$d".odex
    ...
    for e in /system/priv-app/*; do
      ...
      if [ -f /system/priv-app/"$e"/"$e".apk ]; then
        if [ "$(unzip -l /system/priv-app/"$e"/"$e".apk | grep classes.dex)" != "" ]; then
          ...
          echo "- 开始处理$e"
          dex2oat --dex-file=/system/priv-app/"$e"/"$e".apk --compiler-filter="$odex_mode" --instruction-set="$apk_abi" --oat-file="$MODPATH"/system/priv-app/"$e"/oat/"$apk_abi"/"$e".odex
      ...
      for f in /product/app/*; do
        ...
        if [ -f /product/app/"$f"/"$f".apk ]; then
          if [ "$(unzip -l /product/app/"$f"/"$f".apk | grep classes.dex)" != "" ]; then
            ...
            echo "- 开始处理$f"
            dex2oat --dex-file=/product/app/"$f"/"$f".apk --compiler-filter="$odex_mode" --instruction-set="$apk_abi" --oat-file="$MODPATH"/system/product/app/"$f"/oat/"$apk_abi"/"$f".odex
      ...
      for g in /product/priv-app/*; do
        ...
        if [ -f /product/priv-app/"$g"/"$g".apk ]; then
          if [ "$(unzip -l /product/priv-app/"$g"/"$g".apk | grep classes.dex)" != "" ]; then
            ...
            echo "- 开始处理$g"
            dex2oat --dex-file=/product/priv-app/"$g"/"$g".apk --compiler-filter="$odex_mode" --instruction-set="$apk_abi" --oat-file="$MODPATH"/system/product/priv-app/"$g"/oat/"$apk_abi"/"$g".odex
      ...
      for h in /system_ext/app/*; do
        ...
        if [ -f /system_ext/app/"$h"/"$h".apk ]; then
          if [ "$(unzip -l /system_ext/app/"$h"/"$h".apk | grep classes.dex)" != "" ]; then
            ...
            echo "- 开始处理$h"
            dex2oat --dex-file=/system_ext/app/"$h"/"$h".apk --compiler-filter="$odex_mode" --instruction-set="$apk_abi" --oat-file="$MODPATH"/system/system_ext/app/"$h"/oat/"$apk_abi"/"$h".odex
      ...
      for i in /system_ext/priv-app/*; do
        ...
        if [ -f /system_ext/priv-app/"$i"/"$i".apk ]; then
          if [ "$(unzip -l /system_ext/priv-app/"$i"/"$i".apk | grep classes.dex)" != "" ]; then
            ...
            echo "- 开始处理$i"
            dex2oat --dex-file=/system_ext/priv-app/"$i"/"$i".apk --compiler-filter="$odex_mode" --instruction-set="$apk_abi" --oat-file="$MODPATH"/system/system_ext/priv-app/"$i"/oat/"$apk_abi"/"$i".odex
      ...
      for j in /vendor/app/*; do
        ...
        if [ -f /vendor/app/"$j"/"$j".apk ]; then
          if [ "$(unzip -l /vendor/app/"$j"/"$j".apk | grep classes.dex)" != "" ]; then
            ...
            echo "- 开始处理$j"
            dex2oat --dex-file=/vendor/app/"$j"/"$j".apk --compiler-filter="$odex_mode" --instruction-set="$apk_abi" --oat-file="$MODPATH"/system/vendor/app/"$j"/oat/"$apk_abi"/"$j".odex
      ...
      for k in /odm/app/*; do
        ...
        if [ -f /odm/app/"$k"/"$k".apk ]; then
          if [ "$(unzip -l /odm/app/"$k"/"$k".apk | grep classes.dex)" != "" ]; then
            ...
            echo "- 开始处理$k"
            dex2oat --dex-file=/odm/app/"$k"/"$k".apk --compiler-filter="$odex_mode" --instruction-set="$apk_abi" --oat-file="$MODPATH"/system/vendor/odm/app/"$k"/oat/"$apk_abi"/"$k".odex
  ...
  if [ "$odex_module" == "true" ]; then
    ...
    touch "$MODPATH"/module.prop
    {
      ...
      echo "author=柚稚&冷洛"
      echo "description=分离系统软件ODEX，$OS_version$modelversion Android$android_version，编译时间$time"
    } >>"$MODPATH"/module.prop
